from okresult import Result, Ok, Err, safe, fn, map as result_map, unwrap, TaggedError, Panic
import json
...
class TestQuickStart:
    def test_load_user_example(self) -> None:
        ...
        message = parsed.match({
            ...
            "err": fn[Exception, str](lambda e: f"Failed: {getattr(e, 'cause', e)}"),
        })
...
class TestHandlingErrors:
    class TestMapErr:
        def test_transforms_error_value(self) -> None:
            ...
            transformed = err_result.map_err(fn[ValueError, RuntimeError](lambda e: RuntimeError(str(e))))
    ...
    class TestRecovery:
        def test_recovers_from_specific_errors(self) -> None:
            ...
            result = fetch_user("123").match({
                "ok": fn[dict[str, str], Result[dict[str, str], NotFoundError]](lambda user: Result.ok(user)),
...
class TestPanic:
    ...
    def test_and_then_callback_throws_panic(self) -> None:
        try:
            Result.ok(1).and_then(fn[int, Result[int, str]](lambda x: Result.err("error") if False else Result.ok(1 // 0)))
            assert False, "Should have raised Panic"
        except Panic:
            pass
...
class TestUnhandledException:
    def test_automatic_unhandled_exception(self) -> None:
        ...
        result: Result[dict[str, str], Exception] = safe(lambda: parse_json('{"key": "value"}'))
        assert result.is_ok()
